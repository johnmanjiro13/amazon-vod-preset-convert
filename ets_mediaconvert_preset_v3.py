...
# parser.add_argument('-l','--preset-list',action='store',dest='listpresets',help='Feed list of presets ID into script')
parser.add_argument(
...
if not args.interactive and (
    args.region is None or args.etsid is None or args.outputtype is None
):
    parser.print_help()
    exit()
...
def translate_audio(ets_preset_payload, s_audio):
    ...
    if s_audio == '"AAC"':
        ...
        if emf_sample != "auto":
            ...
            print("Auto in setting Sample Rate not supported...defaulting to 48kHz\n")
            AudioSettings["CodecSettings"]["AacSettings"].update(
    ...
    ###PCM/WAV Type
        ...
        if (
            ...
            print("0 and auto channels not supported...defaulting to 2\n")
            emf_channels = "2"
        else:
        ...
        if emf_sample != "auto":
            ...
            print("Auto in setting Sample Rate not supported...defaulting to 44.1kHz\n")
            AudioSettings["CodecSettings"]["WavSettings"].update(
...
def translate_video(ets_preset_payload, s_video):
    ...
    if "Profile" in videodump and s_video != '"mpeg2"':
        ...
        if emf_codec_level == '"1b"':
            ...
            print(
                "WARNING: 1b not supported in AWS Elemental MediaConvert, defaulting to auto, please change to 1 or 1.1 based off bitrate and resolution \n"
            )
        else:
    ...
    if (
        ets_preset_payload["Preset"]["Video"]["CodecOptions"]["InterlacedMode"]
        == "Progressive"
    ):
    ...
    if (
        ets_preset_payload["Preset"]["Video"]["CodecOptions"][
        ...
        emf_enable_color = False
        print(
            "WARNING: Auto in ColorSpaceConversion is not supported in EMF setting ColorSpace on input to Auto and disabling color correction\n"
        )
    ...
    if s_video == '"H.264"':
        ...
        VideoSettings = {
            ...
            "H264Settings": {
                ...
                "GopSize": int(
                    ets_preset_payload["Preset"]["Video"]["KeyframesMaxDist"]
                ),
                "GopSizeUnits": "FRAMES",
    ...
    if ets_preset_payload["Preset"]["Video"]["FrameRate"] == "auto":
        ...
        if float(emf_codec_framerate).is_integer():
            ...
            VideoSettings[xSettings].update(
                {"FramerateNumerator": int(emf_codec_framerate)}
            )
        else:
    ...
    if "MaxBitrate" in videodump:
        if int(ets_preset_payload["Preset"]["Video"]["MaxBitRate"]) > 0:
            ...
            if ets_preset_payload["Preset"]["Video"]["Bitrate"] == '"auto"':
                print(
                    "WARNING: auto not a supported bitrate parameter in EMF setting to default to 5M"
                )
                emf_bitrate = 5000000
                ...
                VideoSettings[xSettings].update({"Bitrate": emf_bitrate})
                emf_max_bitrate = int(
                    ets_preset_payload["Preset"]["Video"]["MaxBitRate"]
                )
                VideoSettings[xSettings].update({"MaxBitrate": emf_max_bitrate})
    else:
...
def translate_container(
    emf_AudioDescription, emf_VideoDescription, s_container, emf_outputgroup, s_video
):
    ...
    if emf_outputgroup == "dash" and s_video == '"H.264"' and s_container == '"fmp4"':
        OutputGroupSettings = {"Settings": {"ContainerSettings": {"Container": "mpd"}}}
        if emf_VideoDescription != "none":
            OutputGroupSettings["Settings"].update(emf_VideoDescription)
        if emf_AudioDescription != "none":
            OutputGroupSettings["Settings"].update(emf_AudioDescription)
    ...
    if emf_outputgroup == "smooth" and s_video == '"H.264"' and s_container == '"fmp4"':
        OutputGroupSettings = {"Settings": {"ContainerSettings": {"Container": "ismv"}}}
        if emf_VideoDescription != "none":
            OutputGroupSettings["Settings"].update(emf_VideoDescription)
        if emf_AudioDescription != "none":
            OutputGroupSettings["Settings"].update(emf_AudioDescription)
    ...
    if emf_outputgroup == "file":
        if s_container == '"ts"' or s_container == '"mpg"':
            ...
            OutputGroupSettings = {
                "Settings": {"ContainerSettings": {"Container": "MXF"}}
            }
            if emf_VideoDescription != "none":
                OutputGroupSettings["Settings"].update(emf_VideoDescription)
            if emf_AudioDescription != "none":
                ...
                "CodecSettings": {
                    ...
                    "FrameCaptureSettings": {
                        ...
                        "FramerateDenominator": int(
                            ets_preset_payload["Preset"]["Thumbnails"]["Interval"]
                        ),
...
if s_audio != "none":
    emf_AudioDescription = translate_audio(ets_preset_payload, s_audio)
else:
    emf_AudioDescription = "none"

emf_PresetSettings = translate_container(
    emf_AudioDescription, emf_VideoDescription, s_container, emf_outputgroup, s_video
)
...
if args.verbose:
    ...
    if args.save:
        print("==================SAVING FILES=========================")
        with open(f"{etsid}.json", "w") as file:
    ...
    if s_video != "none":
        ...
        if args.save:
            with open(f"{etsid}_Thumbnail.json", "w") as file:
